...
namespace SE150557_NguyenHuuDoan
{
    public partial class NewForm : Form
    {
        ...
        /// New function
        static bool digitNumber(string abc)
        {
            bool check = false;
            if(abc.Length >=6 && abc.Length <= 12)
            {
                check = true;
            }
            return check;
        ...
        static bool authorName(string abc)
        {
            bool check = false;
            if(abc.Length >= 10)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                if (digitNumber(txtAccountID.Text))
                {


                    if (DateTime.Parse(txtOpenDate.Text) <= DateTime.Today)
                    {
                        if (checkCapital(txtBranchName.Text))
                        {
                            ...
                            if (checkYear(DateTime.Parse(txtOpenDate.Text)))
                            {
                                ...
                                if (InsertOrUpdate == false)
                                {
                                    ...
                                    txtBranchName.Clear();
                                    mn.LoadUserList(customerRepo.GetAll());
                                }
                                else
                                {
                    ...
                    {
                        MessageBox.Show("Open Date phải bé hơn hiện tại !!");
                    }
                }
                else
                {
